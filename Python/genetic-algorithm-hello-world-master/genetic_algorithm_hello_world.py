...
max_generation = 300  # 最大进化次数
threshold = 0.2  # 当 target 在种群中的比例超过 threshold 时结束


def random_string(n):
    '''生成一个长度为 n 的字符串，即一个个体的基因'''
    return ''.join(random.choice(gene_library) for _ in list(range(n)))
...
def mating(str1, str2):
    ...
    idx = list(range(len(str1)))
    random.shuffle(idx)
    chars = array('u', ' ' * len(str1))
    pos = int(len(idx) / 2)
    for i in idx[:pos]:
        chars[i] = str1[i]
    for i in idx[pos:]:
...
def reach_threshold(population):
    ...
    return len([tup for tup in population
                if tup[1] == 1.0]) * 1.0 / len(population) >= threshold
...
def main(population):
    ...
    population = [(candidate, similarity(candidate, target))
    ...
    avg_similarity.append(
        sum([tup[1] for tup in population]) * 1.0 / len(population))
    ...
    while True:
        ...
        new_population = []
        for j in list(range(int(reproduction_rate * n))):
            child = array('u',
                          mutation(
                              mating(
                                  random_element(population)[0],
                                  random_element(population)[0])))
    ...
    return population, generation, (best_similarity, avg_similarity,
                                    worst_similarity)


population = [random_string(len(target)) for i in list(range(n))]  # 初始种群
